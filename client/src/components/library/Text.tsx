...
import { motion, useScroll, useTransform, Variants, useMotionValue, useMotionTemplate, useSpring } from 'framer-motion';

interface TextProps {
    variant: "default" | "text_reveal_lens" | "text_type_animation" | "animated_text_reveal" | "image_masked_text" | "ultimate_scroll_text" | "rolling_text";
    text: string;
    subText?: string;
    image?: string;
    styles?: {
...
const Text: React.FC<TextProps> = ({ variant, text, subText, image, styles }) => {
    const { color = "#000000", fontSize = "text-6xl", textAlign = "text-center" } = styles || {};

    const commonClasses = `${fontSize} ${textAlign} font-bold`;

    switch (variant) {
        case "text_reveal_lens":
            return <TextRevealLens text={text} className={commonClasses} color={color} image={image} />;
        case "text_type_animation":
            return <TextTypeAnimation text={text} className={commonClasses} color={color} />;
        case "animated_text_reveal":
            return <AnimatedTextReveal text={text} className={commonClasses} color={color} />;
        case "image_masked_text":
            return <ImageMaskedText text={text} image={image} className={commonClasses} />;
        case "ultimate_scroll_text":
            return <UltimateScrollText text={text} className={commonClasses} color={color} />;
        case "rolling_text":
            return <RollingText text={text} className={commonClasses} color={color} />;
        default:
            return (
...
const TextRevealLens = ({ text, className, color, image }: { text: string, className: string, color: string, image?: string }) => {
    ...
    return (
        ...
        >
            ...
            {/* Foreground Text (Revealed by mouse) */}
...
const TextTypeAnimation = ({ text, className, color }: { text: string, className: string, color: string }) => {
...
const AnimatedTextReveal = ({ text, className, color }: { text: string, className: string, color: string }) => {
    ...
    return (
        <motion.div
            style={{ color, display: "flex", flexWrap: "wrap", justifyContent: className.includes("center") ? "center" : className.includes("right") ? "flex-end" : "flex-start" }}
            variants={container}
            initial="hidden"
            whileInView="visible"
            className={className}
...
const ImageMaskedText = ({ text, image, className }: { text: string, image?: string, className: string }) => {
    const fallbackImage = "https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?q=80&w=2898&auto=format&fit=crop";
...
const UltimateScrollText = ({ text, className, color }: { text: string, className: string, color: string }) => {
    ...
    return (
        <div ref={ref} className="relative min-h-[50vh] flex items-center justify-center">
            <motion.h2
...
const RollingText = ({ text, className, color }: { text: string, className: string, color: string }) => {
