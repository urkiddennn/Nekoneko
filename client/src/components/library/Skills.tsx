...
interface SkillsProps {
  ...
  variant?: "default" | "artistic" | "impact" | "bullets" | "brutalist" | "outline_minimal";
}
...
const Skills: React.FC<SkillsProps> = ({
  ...
  const primaryColor = barColor || bulletColor || siteConfig.site_settings.theme.primary;

  const isBrutalist = variant === 'brutalist';
  const isOutlineMinimal = variant === 'outline_minimal';
  const isBulletVariant = variant === 'bullets' || isBrutalist || isOutlineMinimal;
  ...
  const isHex = (color: string) => color && (color.startsWith('#') || color.startsWith('rgb') || color.startsWith('hsl'));
    ...
    return (
      <section>
        <div className="text-center mb-12">
          <h2 className={`text-3xl md:text-4xl font-black text-slate-950 dark:text-white tracking-tight uppercase ${isOutlineMinimal ? 'border-b border-slate-950 dark:border-white inline-block pb-4 px-12' : 'italic'}`}>
            {title}
          </h2>
          {!isOutlineMinimal && (
      ...
      <div className="space-y-16 py-12">
        <div className="flex flex-col items-center text-center space-y-4">
          <div className="flex items-center justify-center gap-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-400">EXPERTISE</span>
            <div className="w-12 h-1 bg-indigo-600" />
          </div>
