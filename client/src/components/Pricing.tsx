...
const Pricing: React.FC<PricingProps> = ({ title, plans, tiers }) => {
    const displayPlans = plans || tiers || [];
    const gridCols = displayPlans.length === 1 ? 'grid-cols-1 max-w-lg mx-auto' : displayPlans.length === 2 ? 'grid-cols-2 max-w-4xl mx-auto' : 'grid-cols-3';
