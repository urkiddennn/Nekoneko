      ...
      <div className="hidden md:flex flex-1 bg-[#111] border-r border-white/[0.04] flex-col p-16 justify-between relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-6 max-w-sm">
          ...
          <p className="text-gray-500 font-medium leading-relaxed">
            Start your journey with our modular block system. pick a theme, add
            your content, and publish instantly.
          </p>
          <div className="flex items-center gap-4 pt-4">
            <div className="flex -space-x-2">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="w-8 h-8 rounded-full border-2 border-black bg-white/[0.04]"
                />
              ))}
            </div>
        ...
        <div className="w-full max-w-sm">
          ...
          >
            {step === "credentials" && (
              <>
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-gray-600 mb-2 px-1">
                    Full Name
                  </label>
                ...
                <div>
                  ...
                  <div className="mt-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                      <div
                        className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider ${password.length >= 8 ? "text-emerald-500" : "text-gray-800"}`}
                      >
                        {password.length >= 8 ? (
                          <Check size={10} />
                        ) : (
                      ...
                      <div
                        className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider ${/[A-Z]/.test(password) ? "text-emerald-500" : "text-gray-800"}`}
                      >
                        {/[A-Z]/.test(password) ? (
                          <Check size={10} />
                        ) : (
                      ...
                      <div
                        className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider ${/[0-9]/.test(password) ? "text-emerald-500" : "text-gray-800"}`}
                      >
                        {/[0-9]/.test(password) ? (
                          <Check size={10} />
                        ) : (
                      ...
                      <div
                        className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider ${/[!@#$%^&*()]/.test(password) ? "text-emerald-500" : "text-gray-800"}`}
                      >
                        {/[!@#$%^&*()]/.test(password) ? (
                          <Check size={10} />
                        ) : (
            ...
            <div className="relative my-8">
              ...
              <div className="relative flex justify-center text-[10px] font-black uppercase tracking-[0.2em]">
                <span className="bg-[#0b0b0b] px-4 text-gray-700">
                  Or continue with
                </span>
              </div>
            </div>
